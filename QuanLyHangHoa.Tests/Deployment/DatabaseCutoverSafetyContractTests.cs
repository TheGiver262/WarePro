...
public sealed class DatabaseCutoverSafetyContractTests
{
    ...
    {
        var prepareSql = DatabaseSchemaScripts.BuildUpgradeSql(8, "1.1.0");
        var finalizeSql = DatabaseSchemaScripts.BuildFinalizeSql(8, "1.1.0");
        ...
        Assert.Contains("Version = 8", finalizeSql, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Sql_write_gate_rejects_legacy_or_unregistered_clients_and_accepts_schema_eight()
    {
        var sql = DatabaseSchemaScripts.BuildFinalizeSql(8, "1.1.0");
        ...
        Assert.Contains("< 8", sql, StringComparison.Ordinal);
