...
## 4. Use case nhập kho và hóa đơn mua
...
flowchart LR
    ...
    subgraph StockIn["Phân hệ nhập kho"]
        direction TB
        UC1(["Lập phiếu nhập kho"])
        UC2(["Quét / nhập serial"])
        UC3(["Gửi duyệt phiếu nhập"])
        UC4(["Duyệt phiếu nhập"])
        UC5(["Ghi sổ phiếu nhập"])
        UC6(["Lập hóa đơn mua<br/>từ phiếu nhập đã ghi sổ"])
        UC7(["Nhập chi tiết hóa đơn mua<br/>theo sản phẩm / đơn giá / số lượng / thuế"])
        UC8(["Nhập tồn đầu kỳ<br/>từ Excel/CSV"])
    end
...
## 5. Use case xuất kho và hóa đơn bán
...
flowchart LR
    ...
    subgraph StockOut["Phân hệ xuất kho"]
        direction TB
        UC1(["Lập phiếu xuất kho"])
        UC2(["Chọn / quét serial<br/>xuất kho"])
        UC3(["Gửi duyệt phiếu xuất"])
        UC4(["Duyệt phiếu xuất"])
        UC5(["Ghi sổ phiếu xuất"])
        UC6(["Lập hóa đơn bán<br/>từ phiếu xuất đã ghi sổ"])
        UC7(["Nhập chi tiết hóa đơn bán<br/>theo sản phẩm / đơn giá / số lượng / thuế"])
    end
