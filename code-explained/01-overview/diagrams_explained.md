...
* `ProductSerial` giữ con trỏ tới dòng nhập gần nhất (`LastStockInLineId`), dòng xuất gần nhất (`LastStockOutLineId`) và dòng chuyển gần nhất (`StockTransferLineId`) để truy vấn nhanh. Các con trỏ này có thể thay đổi sau giao dịch mới; lịch sử đầy đủ vẫn được bảo toàn trong `StockLedger` và chứng từ đã ghi sổ.
